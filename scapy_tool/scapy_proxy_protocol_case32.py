...
PUSH1=ip/TCP(sport=SYNACK.dport, dport=portdst, flags="PA", seq=SYNACK.ack+78, ack=SYNACK.seq + 1) / get
PUSH2=ip/TCP(sport=SYNACK.dport, dport=portdst, flags="PA", seq=SYNACK.ack, ack=SYNACK.seq + 1) / get

send(PUSH1) 
# case 32 the second payload before the first payload

# SEND our ACK-GET request
print "\n[*] Sending ACK-GET packet"
reply,error=sr(PUSH2, multi=True, timeout=0.005)
#reply=sr1(PUSH)
