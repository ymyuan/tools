...
result_raw_synack = sr(IP(dst='192.168.100.100')/TCP(dport=dstport,sport=sportid,flags=2,seq=seqid,options=[('TFO', '')]), verbose = False)
...
#result_raw_fin = sr1(IP(dst='192.168.100.100')/TCP(dport=dstport,sport=sportid,flags=1,seq=cs_sn,ack=sc_sn), verbose = False)
